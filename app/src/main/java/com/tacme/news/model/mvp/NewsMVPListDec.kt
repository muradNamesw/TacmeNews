package com.tacme.news.model.mvp
...
import com.tacme.news.view.viewholders.ListMVPNewsDecVH

import java.util.*

class NewsMVPListDec : Listable {
    ...
    companion object {
        ...
        @Throws(ParseException::class)
        fun fromJson(jsonArray: JSONArray, isList: Boolean?): ArrayList<NewsMVPListDec> {

            val list = ArrayList<NewsMVPListDec>()

            for (i in 0 until jsonArray.length()) {

                try {

                    list.add(NewsMVPListDec(jsonArray.getJSONObject(i), (isList ?: false)))
    ...
    override fun getListItemType(): HolderClass {
        return HolderClass(ListMVPNewsDecVH::class.java, R.layout.list_news_item_mvp)
    }
}
